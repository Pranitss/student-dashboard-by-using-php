...
<script>
    function jobsMenuData() {
      ...
      <div class="col-lg-8">
        <div class="card shadow-sm" >
          <div class="card-header bg-transparent border-0">
            <h3 class="mb-0"><i class="fa fa-phone-square"></i>&nbsp;NOTIFICATIONS</h3>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<h2><a href="#"><span title="Call Centre Agent" id="jobTitle-e79ef11e07e399ed"> ' . $row['branch'] . '</span></a></h2>';
        echo '<div class="new css-ud6i3y eu4oa1w0">';
        echo '<span class="label css-1qj35nq omm eu4oa1w0">' . $row['notidata'] . '</span>';
        echo '</div>';
        echo '</div>';
        echo '<div class="heading6 company_location tapItem-gutter companyInfo">';
       // echo '<span class="companyName">' . $row['notidata'] . '</span>';
        echo '<div class="companyLocation">' . $row['date'] . '</div>';
        ...
        //echo '<div class="attribute_snippet" data-testid="attribute_snippet_testid">'. $row['salary_range'] .' a month</div>';
//         echo '</div>';
...
// echo '</div>';
       ...
       // echo '</svg> &nbsp;'. $row['employment_type'] .'</div>';
        echo '</div>';
