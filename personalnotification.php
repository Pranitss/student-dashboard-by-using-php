...
<script>
    function jobsMenuData() {
      ...
      <div class="col-lg-8">
        <div class="card shadow-sm" >
          <div class="card-header bg-transparent border-0">
            <h3 class="mb-0"><i class="fa fa-phone-square"></i>&nbsp;ADMIN NOTIFICATIONS</h3>
...
if (isset($Email)) {
    $mb = $Email;
    $info = "SELECT * FROM admin_notification WHERE email = '$mb'";
    $result = mysqli_query($con, $info);
    ...
    if ($result) {
        ...
        echo '<h2><a href="#"><span title="Call Centre Agent" id="jobTitle-e79ef11e07e399ed"> ' . $row['name'] . '</span></a></h2>';
        echo '<div class="new css-ud6i3y eu4oa1w0">';
        echo '<span class="label css-1qj35nq omm eu4oa1w0">' . $row['notification'] . '</span>';
        echo '</div>';
        echo '</div>';
